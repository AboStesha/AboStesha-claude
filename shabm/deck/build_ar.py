...
import base64
import json, os
from art import cube_exterior, cube_interior, flank_menu, ICON
import illus
...
PRODUCTS = [
 dict(k="saqee80", ar="صَقيع ٨٠−", la="SAQĪʿ −80", price=MENU["saqee80"]["price_usd_baghdad"],
   lead="كأسٌ مكث يومَين كاملَين في مُجمِّدٍ مخبريّ عند ستٍّ وثمانين تحت الصفر. يُرفَع بمِلقط، ويُصَبّ فيه حليبٌ عند الصفر فيتجمّد على جداره في لحظة، ثم يشقُّ الإسبريسو البياضَ نازلاً. مدّةُ صلاحيّة المشهد: أربعون ثانية.",
   ...
   craft="الكأسُ لا تُبرَّد، بل تُجمَّد: من أربعٍ وعشرين إلى ثمانٍ وأربعين ساعةً في المُجمِّد، وبعضُ المحال "
          "تُبقيها أسبوعاً كاملاً حتى يصير الزجاجُ نفسُه خزّانَ برد يحمل الشرابَ أربعين ثانيةً بلا ثلج. "
          "وهذا يقلب الترتيبَ: لا يُصنَع الطلبُ عند الطلب، بل قبله بيومين. راجع القسم "
          "<span class='n'>١٠</span> لأثر ذلك على المخزون وحجم المُجمِّد.",
...
<div class="grid gA" style="align-items:start">
 <div>
  ...
  وفيها كلمةٌ لا تُقال إلّا للبردِ الذي يُنعش: <b>شَبِم</b> — <b>البردُ اللطيفُ المنعش</b>،
  لا القارسُ ولا اللاذع. بردٌ يُطلَب، لا بردٌ يُتَّقى.</p>
  <div class="g" style="margin-top:1.6rem">
   ...
   <p style="text-align:center;margin-top:.8rem;font-size:var(--s--2);color:var(--ink-4);line-height:1.9">
     ومعناها في كلام العرب بردُ الماءِ الذي يُستطاب — أي الوجهُ المُستحبُّ من البرد،
     وهو المعنى الذي تقوم عليه العلامة.</p>
  </div>
...
def product_block(p):
 ...
 {f'<div class="note" style="margin:0 0 1.4rem">{p["craft"]}</div>' if p.get("craft") else ""}
 <div class="pb-grid">
...
def _prod_art(p):
    """The photo slot for a product, with its drawing standing in until a
    render is dropped into deck/img/."""
    k = p["k"]
    art = illus.PRODUCT_ART.get("jana" if k.startswith("jana") else k)
    return img("p_" + k, p["ar"], p["ar"], cls="sq",
               fallback=art() if art else "")


_prod_row = ('<div class="phgrid g3" style="margin-bottom:2.6rem">'
             + "".join(_prod_art(p) for p in PRODUCTS) + '</div>')
...
_EXT_SHOTS = [("cube_day", "نهاراً — الساعةُ الثانية عشرة، خمسٌ وأربعون درجة"),
              ("cube_night", "ليلاً — المكعّبُ مضاءٌ من داخله"),
              ("cube_dawn", "الفجر — ماءُ الليلة على الإسفلت، والرصيفُ خالٍ"),
              ("cube_corner", "من الزاوية — وجهان وسطحٌ مائل"),
              ("cube_square", "من الأمام — الارتفاعُ يساوي العرض"),
              ("menu_flank", "الجانبُ الأيمن — القائمةُ مجمّدةٌ في الجدار"),
              ("roof_mist", "سقفُ المكعّب — فوّهاتُ الرذاذ تسكب الضبابَ على الحواف، والسطحُ كلُّه ندى"),
              ("frozen_wall", "داخلُ الجدار — ثمرٌ ونعناعٌ ومغرفةٌ مغمورةٌ في اللوح، مقروءةٌ بوضوح"),
              ("ice_bar", "مقترح — بارُ الثلج ومقاعدُه على الضِّلعين الخلفيّ والأيسر")]
...
<div class="grid gA" style="align-items:center">
 <div>{cube_exterior()}{img("face_detail","الفتحاتُ الثلاث","الفتحاتُ الثلاث، عن قرب: شاشةٌ للطلب، وشبّاكٌ للاستلام، وفتحةُ «ارجِع» للأكواب الفارغة.",cls="sq") if has_img("face_detail") else ""}</div>
 <div>
  <p class="lede">ليس مكعّباً هندسيّاً. كتلةُ ثلجٍ <b>مائلة</b>، حوافُّها مأكولةٌ بالذوبان، أضلاعُها غير مستقيمة،
  وقاعدتُها أضيق من قمّتها — كما تفعل كتلةُ ثلجٍ حقيقية تركتها تحت الشمس.
  و<b>مكعّبٌ حقيقيّ</b>: أربعةٌ وعُشران في كلّ ضلع — العرضُ والعمقُ والارتفاع متساوية.
  ثمانيةَ عشرَ متراً مربّعاً لا غير، لكنّها تُقرأ مكعّبَ ثلجٍ من أوّل نظرة، لا لوحاً ولا جداراً.</p>
  <ul class="tk" style="margin-top:1.4rem">
   ...
   <li><b>اسمٌ محفور.</b> <b>شَبِم</b> بالعربية، غائرةً في مادّة الثلج لا مطبوعةً عليها — كبيرةً على الواجهة،
   ومقروءةً من آخر الشارع.</li>
   <li><b>ثلاثُ فتحاتٍ فقط.</b> يميناً شاشةُ الطلب، ويساراً الاستلام — <b>كبيرتان</b>، بحجمٍ يقف عنده الإنسانُ
   ويمدّ يده بصينيّة، لا شقٌّ يتحسّسه. ووسطاً فتحةُ «ارجِع» للأكواب الفارغة، وهي الصغيرة عمداً.</li>
   <li><b>جانبٌ واحدٌ يحمل القائمة.</b> اليمينُ وحده: المنتجاتُ النهائيّة نفسها — <b>بقياسها الحقيقي</b>،
   كما تُقدَّم تماماً — مغمورةٌ في اللوح أثناء صبّه فتبدو مجمّدةً داخله، وفوق كلٍّ منها اسمُه وسعرُه
   محفوران ومُضاءان من الحافة. قائمةٌ ولافتةٌ وفاترينةٌ في قطعةٍ واحدة.</li>
   <li><b>وباردٌ حقّاً، لا في الصورة فقط.</b> مَن يقترب من المكعّب يشعر بالبرد قبل أن يصل إليه، ولذلك
   ثلاثةُ أسبابٍ مجتمعة: <b>القشرةُ الخارجية باردة</b> — جدارٌ مزدوجٌ بتجويفٍ مُهوّى يطرد ٣,٠١٠ واط ويُنزل
   معامِلَ الكسب الشمسي من ٠.٣٥ إلى ٠.٢٧، فيبقى سطحُه دون نقطةِ الندى فيتصبّب ماءً حقيقيّاً لا رذاذاً مرشوشاً؛
   و<b>الإشعاعُ الحراري ينعكس</b> — الجسمُ الواقفُ على مسافةِ ذراعٍ من سطحٍ باردٍ يفقد حرارةً إليه بالإشعاع،
   وهذا ما يُحسّه المارُّ قبل أن يلمس شيئاً؛ و<b>ستارُ الرذاذ</b> من الفوّهات تحت الإفريز يُنزل حرارةَ الهواء
   أمام الفتحات ٨ إلى ١٥ درجة. النتيجةُ نطاقُ بردٍ عرضُه مترٌ ونصفٌ إلى مترين حول الكتلة — لا الشارعُ كلُّه.
   وثمنُه مكتوبٌ في الجدول: <b>١,٤٥٠ دولاراً شهرياً</b> تبريداً وحدَه فوق ٨٠٠ للكهرباء الأساسية، وهو أثقلُ
   بندٍ تشغيليٍّ بعد الرواتب. <b>وتحفُّظٌ للخليج:</b> الرذاذُ التبخيريُّ يعمل في هواء بغداد الجافّ ويكاد
   لا يعمل في رطوبة دبي والدوحة الساحلية — هناك يزداد الندى على السطح ويقلّ أثرُ الرذاذ في الهواء،
   فتُستبدَل الفوّهاتُ بملفّاتِ تبريدٍ في الإفريز، وهي أغلى.</li>
   <li><b>وما عدا ذلك: لا شيء — في بغداد.</b> لا جلوسَ ولا بارَ ولا مظلّةَ ولا نافذةَ عرض. الكتلةُ نظيفة —
   ندىً ودخانُ بردٍ وماءٌ يسيل، وثلاثُ فتحات، واسمٌ محفور. كلُّ إضافةٍ أخرى كانت تُضعِف الفكرة
   وتُثقِل الفاتورة معاً.</li>
  </ul>
  ...
  <div class="note hard" style="margin-top:1.2rem">
   ...
   ولا تنقل الصورة، والفتحاتُ الثلاث صناديقُ تمريرٍ لكلٍّ منها ظهرٌ مغلقٌ من المادّة نفسها — لا نوافذ.
   وهذا قرارٌ يدفع ثمنَه مرّتين لصالحنا: يحفظ البرودةَ عند خمسٍ وأربعين درجة، <b>ويُلغي كلَّ كلفةِ تشطيبٍ
   داخلي</b> — ما لا يُرى لا يُدفَع ثمنُه. التفصيلُ في القسم <span class="n">06</span>.
  </div>
 </div>
</div>
<figure style="margin-top:2.4rem">
 {img("menu_flank_macro", "الدِّرتي مجمّداً في الجدار", "الكأسُ بقياسها الحقيقي، مغمورةً في اللوح، واسمُها وسعرُها محفوران فوقها.", cls="wide") if has_img("menu_flank_macro") else ""}
 {flank_menu([(MENU[k]["ar"], iqd(MENU[k]["price_usd_baghdad"]), t) for k, t in
              [("saqee80","tall"),("qalab","block"),("boba","cup"),
               ("lafaif","rolls"),("jana_piece","fruit"),("radhadh","slush")]])}
 <figcaption>قطاعٌ من الجانب الأيمن، منظوراً من أمامه: <b>١٫٦ متراً عرضاً في ٠٫٦ متراً ارتفاعاً</b> من جدارٍ ارتفاعُه ٤٫٢. ستُّ نسخٍ <b>بالقياس الحقيقي</b> عن المنتجات كما تُقدَّم — أطولُها كأسُ صَقيع بسبعةَ عشرَ سنتيمتراً، لا أكثر —
 مصبوبةٌ داخل اللوح لا مُلصَقةٌ عليه، والأسعارُ محفورةٌ فوقها ومُضاءةٌ من الحافة. الزبونُ يعرف ما يشتريه
 قبل أن يصل إلى الشاشة، ولا يحتاج أحدٌ أن يشرح له. ولا تُقلَّد بلاصق: من أراد نسخَها يحتاج قوالبَ صبٍّ
 ونسخاً دقيقةً لمنتجاتٍ لا يملكها.</figcaption>
</figure>''', cls="d"))

_INT_SHOTS = [("interior", "الداخل — مطبخٌ باردٌ يعمل، لا صالةَ ولا بار"),
...
<p class="sub">٢٥ متراً مربّعاً لا يراها زبونٌ أبداً. ولهذا لا نصرف عليها دولاراً واحداً للشكل.</p>
{_int_row}
<div style="margin-top:2rem">{cube_interior()}</div>
{_svc_row}

<div class="note hard" style="margin-top:2rem">
 <b>القرارُ الذي يوفّر ١٨,٣٠٠ دولاراً: لا هويّةَ في الداخل.</b>
 الكتلةُ مصمتة، والفتحاتُ الثلاث صناديقُ تمريرٍ لكلٍّ منها ظهرٌ مُعتِمٌ من المادّة نفسها — فلا يستطيع أحدٌ
 أن يرى الداخل من أيّ زاوية، ولا في أيّ لحظة. وما لا يُرى لا يُدفَع ثمنُه:
 <b>ألغينا</b> القائمةَ المحفورة في الجدار الداخلي، والشاشاتِ المغروسة في مادّة الثلج، والتشطيبَ المُصمَّم.
 التجهيزُ الداخلي نزل من ٥٢٠$ إلى <b>٣٤٠$ للمتر</b> — ألواحُ غرفةٍ باردة وفولاذٌ وأرضيّةُ إيبوكسي بمصارف —
 والإضاءةُ الداخلية صارت شريطاً صناعياً أبيض. الحفرُ المُضاءُ والاسمُ المنحوت بقيا حيث ينفعان: <b>على الواجهة</b>.
 <br><br>
 والقائمةُ لم تُلغَ، بل انتقلت إلى مكانها الصحيح: <b>شاشةُ الطلب</b>. وهي أفضلُ من الجدار المحفور بثلاثة وجوه —
 تتغيّر بلا نجّار، وتُظهر النفادَ لحظةَ حدوثه، وتعرف أن ترفع صنفاً على آخر.
</div>

<div class="grid g3" style="margin-top:1.8rem">
 <div class="g"><div class="cap">بارد فعلاً، لا مجازاً</div>
  <p>جوفُ الكشك مُبرَّدٌ كغرفةِ تبريد. العاملون يرتدون معاطفَ شتويّة في آب. هذا ليس تفصيلاً جمالياً بل شرطُ
  إنتاج: الغلافُ يقاوم فارقاً حرارياً من ٣٠ إلى ٣٥ درجة في محيطٍ عند ٤٥ إلى ٥٠، ومُجمِّدُ الثمانين
  لا يعمل أصلاً فوق ٣٢ درجةَ محيط. البرودةُ هنا مواصفةٌ هندسية، لا ديكور.</p></div>
 <div class="g"><div class="cap">ثلاثُ فتحاتٍ، ولا كاشير</div>
  <p>على الواجهة ثلاثُ فتحاتٍ مربّعةٍ متساوية ولا رابعة: <b>يميناً</b> شاشةُ لمسٍ يطلب منها الزبون بنفسه،
  و<b>وسطاً</b> فتحةُ «ارجِع» للأكواب الفارغة، و<b>يساراً</b> فتحةُ الاستلام تفتح من جهةٍ واحدة — الموظّف يضع الطلب ويُغلق،
  فيفتحها الزبون من جهته، وفوقها رقمُ الطلب محفوراً في الثلج. لا احتكاكَ ولا كلامَ ولا وقوفَ أمام أحد.</p></div>
 <div class="g"><div class="cap">صورُ التسويق تُصنَع، لا تُصوَّر</div>
  <p>ما دام لا أحدَ يدخل، فالصورُ الداخلية التي تحتاجها الحملاتُ تُنتَج <b>بالذكاء الاصطناعي</b> من مواصفةِ
  التصميم نفسها: لا يومَ تصويرٍ يُغلق الفرع، ولا مصوّرَ ولا إضاءةَ ولا فريقَ إنتاج. وهذا يُخرِج بنداً كاملاً
  من ميزانية ما قبل الافتتاح، ويجعل تجديدَ المحتوى شهرياً بدل موسمياً.</p></div>
</div>

<div class="note" style="margin-top:1.4rem">
 <b>والحاجزُ الحقيقي بقي حيث هو.</b> منافسٌ يستطيع شراءَ صحن لفائف. لا يستطيع بسهولة بناءَ غرفةِ تبريدٍ
 داخل غلافٍ مزدوج فيها مُجمِّدٌ عند ثمانين تحت الصفر ومصنعُ ثلجٍ اتجاهيّ ومولّدةٌ مخصّصة وعقدُ صيانةٍ متخصّص،
 ثم يشغّلها في بغداد بشبكةِ كهرباءٍ تعطي ٨ إلى ١٢ ساعةً في اليوم. الحاجزُ في الميكانيك، لا في التشطيب —
 وهذا بالضبط سببُ عدم الإنفاق على التشطيب.
</div>'''))

# ── ٧ · العلامة ────────────────────────────────────────────────────────────
_MENU_ROWS = [(MENU[k]["ar"], iqd(MENU[k]["price_usd_baghdad"]))
...
_brand_row = ('<div class="phgrid" style="margin-bottom:1.4rem">'
              + "".join(img(k, c, c, cls=cl, fallback=fn())
                        for k, c, fn, cl in _BRAND_ART if cl == "wide") + '</div>'
              '<div class="phgrid g3" style="margin-bottom:2.4rem">'
              + "".join(img(k, c, c, cls=cl, fallback=fn())
...
<div class="grid g3" style="margin-top:2rem">
 <div class="g"><div class="cap">ما يعنيه هذا للإنتاج</div>
  <ul class="tk">
   ...
   <li><b>صَقيع ٨٠−</b> يبلغ <b class="n">{_peak["saqee80"]/30.4:.0f}</b> كأساً يومياً — والكأسُ لا تُبرَّد،
   بل <b>تُجمَّد من أربعٍ وعشرين إلى ثمانٍ وأربعين ساعة</b>، وبعضُ المحال تُبقيها أسبوعاً كاملاً حتى يصير
   الزجاجُ نفسُه خزّانَ برد. وهذا ليس تفصيلاً في الوصفة بل <b>قيدٌ على المخزون</b>: عند ٤٨ ساعة يلزم
   <b class="n">{_peak["saqee80"]/30.4*2:.0f}</b> كأساً دائرةً في المُجمِّدات لا تُباع ولا تُغسَل،
   وعند أسبوعٍ يلزم <b class="n">{_peak["saqee80"]/30.4*7:.0f}</b>. أي أنّ سعةَ اليوم تُحدَّد قبل يومين لا
   الليلةَ الماضية، وأنّ حجمَ المُجمِّد يُقاس بالكؤوس لا باللترات. المُجمِّدتان في الجدول مُحجَّمتان على
   دورة ٤٨ ساعة؛ الأسبوعُ يحتاج ثالثة.</li>
...
def _eqrows():
    ...
    for cat, catlab in _CAT:
        ...
        for e in lines:
            ...
            ico = illus.EQUIP_ART.get(e["key"], "")
            out.append([f'<span class="eqline">{ico}<span>{e["ar"]}</span></span>',
...
<div class="note hard" style="margin-top:1.6rem">
 <b>من ٥٣٨,٩٤٤$ إلى {usd(FLAG["capex"]["total"])}. أي أقلّ بـ٤٩٪.</b>
 الرقمُ الأوّل كان يعني نحو <span class="n">$21,500</span> للمتر المربّع في كشكٍ من ٢٥ متراً،
 وهو رقمٌ لا يُصدَّق. نزل على ستّ مراحل، كلُّها قرارٌ لا خصمُ مساومة:
 <ol class="st" style="margin-top:1rem">
  <li><b>عُدّةُ الامتياز إلى مكانها.</b> وثيقةُ الإفصاح ودليلُ التشغيل ومنهجُ التدريب بـ<span class="n">$45,000</span>
  كلفةُ الشركة المانحة، تُكتَب مرّةً وتستعملها كلُّ الفروع — لا كلفةَ فرعٍ واحد.</li>
  <li><b>مصنعُ ثلجٍ بوحدةٍ لا بثلاث.</b> الطلبُ الناضج ٢٤ كأساً في اليوم، والوحدةُ تعطي ٣٠ إلى ٤٢.
  <span class="n">$34,000</span> ← <span class="n">$6,500</span>.</li>
  <li><b>لا هويّةَ داخل كتلةٍ لا تُرى.</b> التجهيزُ الداخلي بمواصفة مطبخٍ يعمل لا غرفةٍ تُصوَّر.</li>
  <li><b>المساحةُ نفسها.</b> ٢٥ متراً ← <b>١٨</b>، والغلافُ ٩٨ متراً مربّعاً ← <b>٧٥</b>.
  المترُ الزائد يُدفَع ثمنُه مرّتين: في البناء وفي تبريده كلَّ شهر.</li>
  <li><b>حذفُ ما أُضيف ثم لم يخدم الفكرة.</b> البارُ والمقاعد، والمظلّة، ونافذةُ العرض على الصحن —
  <span class="n">$16,586</span> عادت. الكتلةُ النظيفة أقوى بصرياً وأرخص.</li>
  <li><b>مراجعةُ كلِّ سطرٍ في الجدول.</b> الغلافُ من المطحنة مباشرةً بنقشٍ واحد، والهيكلُ يُصنَّع في بغداد
  لا يُستورَد، ومكنةُ إسبريسو تجارية لا فاخرة، وطرفا طلبٍ بنظام أندرويد لا عتادُ نقطةِ بيعٍ كامل،
  والهويّةُ ما بقي منها إخراجٌ للطباعة لأنّ النظامَ نفسه مبنيٌّ في هذا الملفّ.
  نحو <span class="n">$38,600</span> من ثلاثةٍ وعشرين سطراً.</li>
 </ol>
 وأُضيف بندٌ واحدٌ بكامل كلفته: <b>جدارُ القائمة</b> بـ<span class="n">$7,800</span> — النسخُ بالقياس
 الحقيقي مصبوبةً في اللوح. وهو أرخصُ من لافتةٍ وقائمةٍ وفاترينةٍ منفصلة، ولا يُقلَّد بلاصق.
 <br><br>
 <b>وما دُفع ثمنُه بصدق:</b> نزعُ الجلوس نزع معه مناسبةَ البقاء، فخُفِّض أساسُ العمليات اليومية من ٣٠٠
 إلى <b>٢٧٠</b> في النموذج. الاستردادُ <b class="n">{FLAG["payback_months"]:.1f}</b> شهراً
 والعائدُ الداخلي <b class="n">{FLAG["irr_5y"]:.0f}%</b>.
</div>
...
<div class="grid g2" style="margin-top:2rem;align-items:start">
 <div class="g solid">
  ...
  <div class="phgrid" style="margin:1.4rem 0">
   {img("ice_bar", "صيغةُ الخليج — حلقةُ جلوسٍ من الثلج حول الكتلة",
        "الفرعُ الخليجيُّ ليس كشكَ بغداد مكبَّراً: كتلةٌ أكبر، وحولها بارٌ ومقاعدُ من المادّة نفسها، "
        "وضبابٌ باردٌ يسقط على الجالسين. اثنان وعشرون مقعداً وشرفةٌ اثنان وثلاثون متراً مربّعاً — "
        "وهي في أرقام دبي والرياض والدوحة أصلاً، لا إضافةً عليها.", cls="wide")
    if has_img("ice_bar") else ""}
  </div>
  <div class="note" style="margin:1.4rem 0">
   <b>لماذا يجلس الناسُ في الخليج ولا يجلسون في بغداد.</b> الفرقُ ليس ذوقاً بل حسابٌ في النموذج:
   صيغةُ التكرار الخليجية <b>٢٥ متراً مربّعاً و٢٢ مقعداً و٣٢ متراً شرفةً</b>، وأثاثُها <b>٩,٠٠٠ دولارٍ</b>
   داخلَ جدول كلفتها، وإيجارُها <b>١,٩٠٠ دولارٍ</b> شهرياً — كلُّه محسوبٌ في أرقام دبي والرياض والدوحة
   في هذه الصفحة وما بعدها. أمّا بغداد فكشكُ أخذٍ‑ومضيّ بثمانيةَ عشرَ متراً وصفرِ مقاعد، وقد
   <b>خُفِّض أساسُ عملياتها من ٣٠٠ إلى ٢٧٠ يومياً</b> ثمناً لذلك، صراحةً لا ضمناً.
   والسببُ أنّ الخليجَ يشتري <b>المكوث</b>: مركزٌ تجاريٌّ أو واجهةٌ بحرية يُقاس نجاحُها بالدقائق لا بالكؤوس،
   والإيجارُ فيها يُدفع على الأثر لا على المتر. وبغدادُ اليوم تشتري <b>السرعة</b>، والرصيفُ فيها لا يحتمل
   حلقةَ مقاعدَ ولا تصريحَها.
   <b>والتحفّظ الذي يبقى:</b> بارُ الثلج يذوب — بندٌ استهلاكيٌّ يُعاد صبُّه، لا أصلٌ يُستهلَك على خمس سنوات؛
   وهو مُدرَجٌ في الأثاث لا في بندٍ مستقلّ، وسيُفرَد حين تُطلَب عروضُ الأسعار.
  </div>
